  ...
  return (
    ...
    >
      ...
      <span className="asset-copy">
        ...
        <small>{layerNames[0] || expectedFilename(asset)}</small>
      </span>
...
export default function AssetList() {
  ...
  const filtered = useMemo(
    () =>
      assets.filter((asset) =>
        `${asset.id} ${expectedFilename(asset)} ${(refs[asset.id] || []).join(" ")}`
          .toLowerCase()
          .includes(normalizedQuery),
      ),
    [assets, refs, normalizedQuery],
  );
  ...
  const downloadSelected = async () => {
    ...
    try {
      ...
      const anchor = Object.assign(document.createElement("a"), {
        href: url,
        download: replacements[selectedId]?.name || expectedFilename(selected),
      });
  ...
  return (
    <section className="asset-panel panel" aria-label="Animation assets">
      <div className="panel-heading">
        <div>
          <p className="eyebrow">Assets</p>
          <h2>{assets.length} images</h2>
        </div>
        <span
          className={`count-pill ${Object.keys(replacements).length ? "is-edited" : "is-clean"}`}
        >
          {Object.keys(replacements).length} edited
        </span>
      </div>
      <label className="search-field">
        ...
        <input
          ...
          placeholder="Search assets or layers"
        />
      </label>
      <div className="asset-list" role="list">
        {filtered.map((asset) => (
          <AssetRow
            key={asset.id}
            asset={asset}
            layerNames={refs[asset.id] || []}
            replacement={replacements[asset.id]}
        ...
        {!filtered.length && (
          <div className="empty-list">
            {assets.length
              ...
              : "This animation has no image assets."}
          </div>
        )}
      </div>
      <div className="asset-actions">
        <Button
          ...
          disabled={
            !selected ||
            !String(
              replacements[selectedId]?.dataUrl || selected?.p,
        ...
        <Button
          variant="ghost"
          icon={Trash2}
          disabled={!replacements[selectedId]}
      ...
      {selected && (
        <p className="selected-info">
          Selected <strong>{expectedFilename(selected)}</strong>
          {replacements[selected.id] &&
            ` · ${formatBytes(replacements[selected.id].size)}`}
        </p>
      )}
    </section>
  );
